...
describe("agent issue mutation checkout ownership", () => {
  ...
  it.each([
    ...
    // ANT-1076: peer-agent comment is now allowed; positive coverage lives in
    // `issue-comment-auth-split.test.ts`. Other mutations (patch, delete,
    // document, work product, attachments) remain strict — that's what this
    // table guards.
    [
      "document upsert",
      (app: express.Express) =>
  ...
  // ANT-1076: comment row removed from the table — peer-agent comments are
  // now allowed on todo/blocked/in_progress same-company issues. Positive
  // coverage in `issue-comment-auth-split.test.ts`.
  it.each([
