...
/**
 * Helper to assert wake payload data is present in the human-readable
 * `message` string emitted by the ANT openclaw_gateway adapter.
 *
 * ANT fork delivers the wake payload as a JSON code-fence inside the
 * `message` field (not `payload.paperclip.wake.*`) to satisfy the
 * gateway agent-params schema (commits 7518696c / 4af590cf / 6c9e639a).
 */
function expectWakePayloadInMessage(payload: Record<string, unknown> | undefined, expected: Record<string, unknown>) {
  const message = String(payload?.message ?? "");
  const fenceMatch = message.match(/```json\n([\s\S]*?)\n```/);
  if (!fenceMatch) {
    throw new Error(
      `Expected wake payload JSON fence in payload.message but found none. Message preview: ${message.slice(0, 200)}`,
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new Error(`Failed to parse wake JSON fence: ${(error as Error).message}\nRaw: ${fenceMatch[1].slice(0, 200)}`);
  }
